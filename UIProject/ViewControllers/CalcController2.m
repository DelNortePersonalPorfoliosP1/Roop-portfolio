...
-(void)calculateAnswer  // method to perform calculation
{
    ...
    switch(mathOp)
    {
        ...
        case MODULO:
            ...
            break;
       case POWER:
            calcAnswer = (double)pow((double)arg1,(double)arg2);
            break;
        case -1:
            calcAnswer = arg1;
    }
...
-(IBAction)pressDButton:(id)sender {  // Interface Builder action for (9)
    NSString *keyNumber = @".";
    [self concatCalcAreaLabel:keyNumber];
}
-(IBAction)pressNButton:(id)sender {  // Interface Builder action for (9)
    NSString *keyNumber = @"-";
    [self concatCalcAreaLabel:keyNumber];
}
